...
public:
  ...
  ///         // set [-1,2] x [-1,2]
  ///         Eigen::Vector3d p0(-1, -1);
  ///         Eigen::Vector3d p1(2, 2);
  ///         auto mesh = Rectanglemesh::Mesh::create(MPI_COMM_WORLD, {p0, p1},
  ///         {8,
  ///         8});
  /// @endcode
  static mesh::Mesh create(MPI_Comm comm, const std::array<Eigen::Vector3d, 2>& p,
...
private:
  // Build mesh
  static mesh::Mesh build_tri(MPI_Comm comm, const std::array<Eigen::Vector3d, 2>& p,
                              std::array<std::size_t, 2> n,
                              const mesh::GhostMode ghost_mode,
                              std::string diagonal = "right");

  static mesh::Mesh build_quad(MPI_Comm comm, const std::array<Eigen::Vector3d, 2>& p,
                               std::array<std::size_t, 2> n,
                               const mesh::GhostMode ghost_mode);
};
} // namespace generation
